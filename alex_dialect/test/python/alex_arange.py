...
BUILD_DIR = (
    "/home/cflux/Documents/Jeoffrey/01.Projects/01.MLIR/MLIR/"
    "alex_dialect/build"
)
...
MLIR_FILE = TEST_DIR / "arange" / "arange_ngtive.mlir"
...
def ctype_for(mlir_type):
    ...
    if key not in MLIR_TO_CTYPE:
        raise RuntimeError(
            f"Don't know the ctypes equivalent of '{key}'"
        )
...
def get_all_funcs(module):
    ...
    if not funcs:
        raise RuntimeError(
            "No func.func found in this module"
        )

    return funcs


def read_nested(data_ptr, sizes, strides, offset,
                dim=0, base_index=()):
    if dim == len(sizes):
        flat_index = (
            offset
...
def test_arange():
    ...
    # torch.arange(5)
    expected = torch.arange(5.0, 0.0, -1.0)
